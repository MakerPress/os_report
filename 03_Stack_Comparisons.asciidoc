...
And to wrap up with the 11 technologies that are offered to Bluehost subscribers, we have the following miscellaneous suite of tools. Phpbb is a bulletin board tool, Simple Machine Forum is a forum builder tool, Mediawiki is a wiki that allows users to edit content, Moodle is an Educational tool to manage class work, and Coppermine is a photo management tool.
