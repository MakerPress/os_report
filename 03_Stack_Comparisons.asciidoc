...
The growth of open source tools during the past several years is not an easy measure to obtain. The following analysis will contain a combination of job data along with user data from Bluehost. But we are not going to look at all Open Source trends here, but rather the components that help a SMB get a presence on the Web. The technology components that make the Web stack for Bluehost's SMBs are shown in the following two charts.  The first one is the advertised Job postings for the 11 technologies we looked at, and the second chart is the usage by Bluehost customers for the same 11 components. 
